...
play = Button('play0.png', 200, 250, 0, 300, 100)
...
while game:
        ...
        if menu != True:
            ...
            for e in events:
                if e.type == MOUSEBUTTONDOWN: 
                    if play.collidepoint(mouse_x, mouse_y) and e.button == 1:
                        game_status = 1
    ...
    for e in events:
        if e.type == QUIT:
            game = False
            counters = {'player1': counter1,'player2': counter2}
            with open('counters.json', 'w') as file:
                json.dump(counters, file, ensure_ascii=False, indent=4)
    display.update()   